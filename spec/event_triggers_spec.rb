...
describe AllTrigger do 
	...
	before :each do 
		trigger1 = TickTrigger.new
		trigger2 = AttrTrigger.new( :seconds => 0.5 )
	...
	it "should match if all included triggers match" do 
		@trigger.match?( TickEvent.new(0.5) ).should be_true
	end
	
	it "should not match if any of the included triggers does not match" do 
		@trigger.match?( TickEvent.new(0.1) ).should be_false
	end
