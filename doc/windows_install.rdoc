...
- {SDL_gfx}[http://www.ferzkopp.net/joomla/content/view/19/14/].  This is
  the trickiest dependency, because there is currently no package for a
  precompiled version of SDL_gfx. If you are able to compile this yourself,
...
(except SDL_gfx, as noted above).  We'll use a new directory,
<code>C:\\dlls\\</code>.  Add this location to your system PATH -- 
Windows searches the PATH for libraries as well as executables.
...
the libraries, and '\dlls' (line 10) with the path where you put the DLLs.
...
The -L parameter for LIBRUBYARG_SHARED may be different if you installed Ruby to a
path other than C:/ruby.
