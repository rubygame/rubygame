...
--- Queue#_wait
    ((*For internal use only!*)) Wait for an event of any type, and return that
    event (as a Rubygame event).

--- Queue#wait( *classes )
    Wait indefinitely for an event generated by Rubygame, and return the first 
    one that occurs.
        * ((|classes|)): zero or more Event class. All events except those of 
          the given types will be ignored. If no types are given, the first
          event of ((*any*)) type will be returned.
    ((*NOTE*)): types other than the normal Rubygame events will be ignored,
    as they will not be generated by Rubygame. If you give any arguments, but
    none of them are Rubygame event classes, (({nil})) will be returned 
    immediately.
